...
void abmConsultas(eAlquiler alquiler[],int tamAlquiler,eJuego juegos[],int tamJuego,eCliente clientes[],int tamClientes)
{
    ...
    do
    {
        ...
        case 5:
            listarJuegosxCliente(alquiler,tamAlquiler,juegos,tamJuego,clientes,tamClientes);
            system("pause");
            break;
...
void listarJuegosxCliente(eAlquiler alquileres[], int tamAlqui, eJuego juegos[], int tamJuegos, eCliente cliente[],  int tamClientes)
{
    int idCliente;
    int idJuego;
    char apellido[51];
    ...
    idCliente = seleccionarCliente(cliente, tamClientes);
    cargarCliente(cliente, tamClientes, idCliente, apellido, nombre);



    printf("\n--Alquileres de %s %s--\n", apellido, nombre);
    for(int i=0; i<tamAlqui; i++){
        if(alquileres[i].isEmpty == 0 && alquileres[i].idCliente == idCliente){
            idJuego = alquileres[i].idJuego;
            for(int j=0; j<tamJuegos; j++){
                if(juegos[j].isEmpty == 0 && juegos[j].idJuego == idJuego){
                    printf("\nAlquiler %d:\n", alquileres[i].id);
